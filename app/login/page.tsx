    ...
    <div className="flex h-[calc(100vh-1.5rem-2rem)] w-full bg-white justify-start items-center pt-8 relative flex-col">
        ...
        <div className="flex w-full h-full px-16 lg:px-28 flex-col items-center gap-8 pt-12">
            ...
            <div className="flex w-full lg:w-2/3 justify-between">
                ...
                <Link href={'/admin'} className="text-white bg-tertiary px-3 py-2 text-xl flex h-fit mt-4 rounded-xl">Sign Up</Link>
            </div>
            <div className="flex w-full lg:w-2/3  bg-accent flex-col  justify-between p-16 gap-4 rounded-lg"  >
                <div className="flex w-full gap-8">
                <div className="flex w-full flex-col">
                    <h1 className="text-2xl">Username</h1>
                    <input type="text" value={username} onChange={e=>usernameupdate(e.target.value)} placeholder="username" name="usernane" className="bg-gray-200 rounded-xl h-12 p-2 w-full"/>
                </div>
                <div className="flex w-full flex-col">
                    <h1 className="text-2xl">Password</h1>
                    <input type="password" value={password} onChange={e=>passwordupdate(e.target.value)}  placeholder="password" name="password" className="bg-gray-200 rounded-xl h-12 p-2 w-full"/>
                </div>
                </div>
